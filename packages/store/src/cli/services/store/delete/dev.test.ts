...
describe('deleteDevStore', () => {
  test('requests deletion and renders success after polling sees the plan become cancelled', async () => {
    ...
    expect(renderSuccess).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        body: expect.arrayContaining([expect.stringContaining('confirmed the store was deleted')]),
      }),
    )
  ...
  test('rewrites the backend CLI store creation gate as a deletion-specific error', async () => {
    mockBusinessPlatformRequests({
      mutationResult: {
        deleteAppDevelopmentStore: {
          success: false,
          userErrors: [
            {
              code: null,
              field: ['base'],
              message: 'Crear una tienda desde la CLI de Shopify aún no está habilitado para tu organización.',
            },
          ],
        },
      },
    ...
    await expect(deleteDevStore(defaultOptions)).rejects.toThrow(
      'Failed to delete development store: Deleting a development store from the Shopify CLI is not yet enabled for your organization.',
    )
