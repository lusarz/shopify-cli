...
export async function deleteDevStore(options: DeleteDevStoreOptions): Promise<void> {
  ...
  if (userErrors && userErrors.length > 0) {
    const messages = userErrors.map((error) => deleteDevelopmentStoreUserErrorMessage(error.message)).join(', ')
  ...
  if (options.json) {
    ...
    renderSuccess({
      ...
      body: ['Shopify accepted the deletion request and confirmed the store was deleted.'],
    })
  } else {
    renderWarning({
...
function deleteDevelopmentStoreUserErrorMessage(message: string): string {
  // The backend delete operation currently reuses the store-create CLI gate and localized
  // message. Keep the CLI output delete-specific so users understand which operation failed.
  if (isCliStoreCreateGateMessage(message)) {
    return 'Deleting a development store from the Shopify CLI is not yet enabled for your organization.'
  }

  return message
}

function isCliStoreCreateGateMessage(message: string): boolean {
  return [
    'Creating a store from the Shopify CLI is not yet enabled for your organization.',
    'Crear una tienda desde la CLI de Shopify aún no está habilitado para tu organización.',
  ].includes(message)
}
